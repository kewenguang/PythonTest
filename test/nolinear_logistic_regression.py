...
filename = "nolinear_logistic_regression_dataset.txt"
...
def gradAscent(XX, y, l):
    ...
    print('Cost: \n', cost)
    # 最优化 costFunctionReg   initial_theta值是初始值猜测
    # 有了梯度，但是步长在哪呢？？  这个怎么看？  学习速率是不是自己调整的？？
...
def plotBestFit(data,res2,X,accuracy,l,axes):  #画出最终分类的图
    ...
    xx1, xx2 = np.meshgrid(np.linspace(x1_min, x1_max), np.linspace(x2_min, x2_max)) #np.linspace默认情况下会生成50个
    h = sigmoid(map_feature(xx1.ravel(), xx2.ravel()).dot(res2.x))
    h = h.reshape(xx1.shape)
    if axes == None:
        axes = plt.gca()
    axes.contour(xx1, xx2, h, [0.5], linewidths=1, colors='g');#画等高线，并只画出h值为0.5的线 [0.6,0.7]则表示画出0.6和0.7的等高线
...
def plotData(data, label_x, label_y, label_pos, label_neg, axes):
    ...
    if axes == None:
        axes = plt.gca()
    axes.scatter(data[pos][:, 0], data[pos][:, 1], marker='+', c='k', s=60, linewidth=2, label=label_pos)#c = 'k'代表颜色是黑色
